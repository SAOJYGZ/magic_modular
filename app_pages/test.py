...
#         agg = df_m.groupby(['周期', 'counterparty'], as_index=False)[['名义本金', '保证金']].sum()
#         if '累计' in metric_type:
#             agg = agg.sort_values(['counterparty', '周期'])
#             agg['累计名义本金'] = agg.groupby('counterparty')['名义本金'].cumsum()
#             agg['累计保证金'] = agg.groupby('counterparty')['保证金'].cumsum()
#             melt_cols = ['累计名义本金', '累计保证金']
#         else:
#             melt_cols = ['名义本金', '保证金']

#         long = agg.melt(
#             id_vars=['周期', 'counterparty'],
...
#         # 堆叠直方图 (按指标分面)
#         st.subheader(f"{selected_product} {metric_type}（按{freq}）—堆叠直方图")
#         fig_stack = px.bar(
#             long,
#             x='周期', y='值',
#             color='counterparty',
#             facet_row='指标',
#             barmode='stack',
#             labels={'counterparty': '交易对手方', '值': metric_type},
#             title=f"{selected_product} {metric_type} 各交易对手方堆叠直方图"
#         )
#         fig_stack.update_layout(legend_title_text='交易对手方')
#         st.plotly_chart(fig_stack, use_container_width=True)

#         # 分组直方图 (按指标分面)
#         st.subheader(f"{selected_product} {metric_type}（按{freq}）—分组直方图")
#         fig_group = px.bar(
#             long,
#             x='周期', y='值',
#             color='counterparty',
...
#             barmode='group',
#             labels={'counterparty': '交易对手方', '值': metric_type},
#             title=f"{selected_product} {metric_type} 各交易对手方分组直方图"
#         )
#         fig_group.update_layout(legend_title_text='交易对手方')
...
#         frames = []
#         for t in points:
#             sel = df_o[(df_o['startDate_dt'] <= t) & ((df_o['tradeTerminationDate_dt'].isna()) | (df_o['tradeTerminationDate_dt'] >= t))]
#             if sel.empty:
#                 continue
...
#         df_dyn = pd.concat(frames, ignore_index=True)
#         long = df_dyn.melt(
#             id_vars=['周期', 'counterparty'],
#             value_vars=['名义本金', '保证金'],
#             var_name='指标',
...
#         st.subheader(f"{selected_product} 期末存续（按{freq}）—堆叠直方图")
#         fig_stack = px.bar(
#             long,
#             x='周期', y='值',
#             color='counterparty',
#             facet_row='指标',
#             barmode='stack',
#             labels={'counterparty': '交易对手方', '值': '值'},
#             title=f"{selected_product} 期末存续 各交易对手方堆叠直方图"
#         )
#         fig_stack.update_layout(legend_title_text='交易对手方')
#         st.plotly_chart(fig_stack, use_container_width=True)

#         st.subheader(f"{selected_product} 期末存续（按{freq}）—分组直方图")
#         fig_group = px.bar(
#             long,
#             x='周期', y='值',
#             color='counterparty',
...
#             barmode='group',
#             labels={'counterparty': '交易对手方', '值': '值'},
#             title=f"{selected_product} 期末存续 各交易对手方分组直方图"
#         )
#         fig_group.update_layout(legend_title_text='交易对手方')
#         st.plotly_chart(fig_group, use_container_width=True)
